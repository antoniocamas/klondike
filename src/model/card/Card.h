...
public:
    ...
    Card(unsigned int number, shared_ptr<Suit> suit);
    virtual ~Card(){};
    Card & operator=(Card other);
    bool operator==(Card otherCard) const;
    bool operator!=(Card otherCard) const {return !((*this) == otherCard);};

    inline const Suit& getSuit() const {return *this->suit.get();}
    ...
    inline bool isSameColor(Card other) const{return other.suit->isSameColor(*this->suit.get());}
